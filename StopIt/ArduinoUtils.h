...
/// register.
class ShiftRegister {
  DigitalOutputPin dataPin;
  DigitalOutputPin clockPin;
  uint8_t currentValue = 0;

public:
  ShiftRegister(uint8_t dataPin, uint8_t clockPin):
    dataPin(dataPin), clockPin(clockPin) {}

  /// Dumps the current value as binary over serial.
  void dump() {
    Serial << "current values: ";
    for (int i = 7; i >= 0; --i) {
      Serial << ((currentValue >> i) & 1);
    }
    Serial << "\n";
  }

  /// Sets the specified `pin` to `high` in the internal representation of the
  /// shift register's contents.
  /// NOTE: This does not write out to the shift register. After setting or
  ///       clearing all bits, you must call `write()` to send the bits to the
  ///       shift register.
  void high(uint8_t pin) {
    currentValue |= (1 << pin);
  }

  /// Sets the specified `pin` to `low` in the internal representation of the
  /// shift register's contents.
  /// NOTE: This does not write out to the shift register. After setting or
  ///       clearing all bits, you must call `write()` to send the bits to the
  ///       shift register.
  ...
  /// Writes the internal representation to the shift register.
  void write() {
    shiftOut(dataPin.pinNum, clockPin.pinNum, MSBFIRST, currentValue);
  }

  /// Writes 0 to the shift register and clears the internal representation.
  void clear() {
    currentValue = 0;
    write();
  }
